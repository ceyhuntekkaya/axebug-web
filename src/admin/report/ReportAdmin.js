...
export default function ReportAdmin(props) {
    ...
    let {examId, studentId} = props;
    const [reportData, setReportData] = useExam(null);
    const [studentScore, setStudentScore] = useStudentWork(scoreModel);

    useEffect(() => {
        document.body.style.backgroundColor = 'white';
        setReportData("createReport", {studentId: studentId, examId: examId})
        setStudentScore("studentScore", studentId)
                ...
                reportData ?
                    <div className='container m-5'>
                        <div className='row text-white bg-dark p-4'>
                            <div className='col-3 text-white bg-dark d-flex justify-content-center'><h2>
                                <b>{reportData.exam ? reportData.exam.name ? reportData.exam.name : null : null}</b>
                            </h2></div>
                            <div className='col-3 text-white bg-dark d-flex justify-content-center'><h2>
                                <b>{reportData.student.name} {reportData.student.surname}</b></h2></div>
                        </div>
                        ...
                        {studentScore ?
                            <React.Fragment>
                                {parseInt(studentScore.readScore) > -1 ? <div className='row border border-light mt-2'>
                                    <div className='col-2'><h4><b>Reading : {parseInt(studentScore.readScore)}</b></h4>
                                    </div>
                                    <div className='col-10'>{getText('Reading', parseInt(studentScore.readScore))}
                                    </div>
                                </div> : null}
                                {parseInt(studentScore.listeningScore) > -1 ? <div className='row border border-light'>
                                    <div className='col-2'><h4><b>Listenig : {parseInt(studentScore.listeningScore)}</b>
                                    </h4></div>
                                    <div className='col-10'>{getText('Listenig', parseInt(studentScore.listeningScore))}
                                    </div>
                                </div> : null}
                                {parseInt(studentScore.writeScore) > -1 ? <div className='row border border-light'>
                                    <div className='col-2'><h4><b>Writing : {parseInt(studentScore.writeScore)}</b></h4>
                                    </div>
                                    <div className='col-10'>{getText('Writing', parseInt(studentScore.writeScore))}
                                    </div>
                                </div> : null}
                                {parseInt(studentScore.speakingScore) > -1 ? <div className='row border border-light'>
                                    <div className='col-2'><h4><b>Speaking : {parseInt(studentScore.speakingScore)}</b>
                                    </h4></div>
